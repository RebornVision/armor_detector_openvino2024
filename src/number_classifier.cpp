...
NumberClassifier::NumberClassifier(
  const std::string & model_path, const std::string & label_path, const int detect_color,
...
void NumberClassifier::extractNumbers(const cv::Mat & src, std::vector<Armor> & armors)
{
  ...
  for (auto & armor : armors) {
    ...
    cv::Point2f lights_vertices[4] = {armor.bl, armor.tl, armor.tr, armor.br};
    ...
    // // 防止同色灯条影响二值化
    // if (detect_color_)
    //   channels[2] = cv::Mat::zeros(number_image.rows, number_image.cols, CV_8UC1);  // red channel BGR
    // else
    //   channels[0] = cv::Mat::zeros(number_image.rows, number_image.cols, CV_8UC1);  //blue channel
    // cv::merge(channels, number_image);
