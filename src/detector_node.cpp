...
std::string model_path_yolo = pkg_path+"/model/mobilenetv3_last_int_all_new/last.xml";
  // 配置检测器的护甲参数
  Detector::ArmorParams a_params = {
    ...
    .binary_threshold_light = declare_parameter("armor.binary_threshold_light",80)
 
    };

    

  auto detector = std::make_unique<Detector>(model_path_yolo,detect_color, a_params);

  // Init classifier
  
  auto model_path = pkg_path + "/model/mlp.onnx";
  auto label_path = pkg_path + "/model/label.txt";
  double threshold = this->declare_parameter("classifier_threshold", 0.6);
  ...
  detector->classifier = std::make_unique<NumberClassifier>(
    model_path, label_path, detect_color, threshold, ignore_classes);
  ...
  const sensor_msgs::msg::Image::ConstSharedPtr & img_msg)
{
  ...
  detector_->a.binary_threshold_light = get_parameter("armor.binary_threshold_light").as_int();
  // 更新检测器的参数
  ...
  //RCLCPP_INFO(this->get_logger(), "Latency: %lf", latency);
